...
QE 中 `epsil=.true.` 请求的是特定条件下的电场响应分支。输出张量并不自动证明 polar correction 可用于所有后处理：需要检查体系是否适合绝缘体电场响应、tensor symmetry 是否合理、warnings 是否解释、`dynmat.x`/`matdyn.x` 是否读取同一数据链，以及 LO-TO 方向和 ASR/zasr 设置是否记录。异常的 Born charge 或 dielectric tensor 不是单一信号：它可能是真实强极化/动态电荷转移，也可能来自小带隙、部分占据、赝势、结构畸变、k mesh、response 未收敛或 non-analytic 文件链混用。
...
- 真实物理响应：强极性键、动态电荷转移、低对称结构或方向相关长程电场导致 tensor 或 LO-TO splitting 明显。
- 数值误差：SCF、`tr2_ph`、k mesh、cutoff 或 q-grid 不足导致 tensor 不稳定。
- 模型误差：functional、pseudopotential、band gap 或 occupation 设置改变响应。
- 文件链误差：Born/dielectric tensor、IFC、`fildyn`、`flfrc` 或 `matdyn.x` non-analytic 设置来自不同结构或不同计算。
- 适用条件错误：金属、部分占据或电场响应条件不满足时仍按极性绝缘体解释。
...
| tensor symmetry 或 sum rule 异常 | 结构对称性、response 收敛或文件链风险 | symmetry output、Born tensor 总和、warning |
| LO-TO 分裂未体现 | non-analytic correction 未使用或方向未记录 | dynmat/matdyn 设置 |
| `matdyn.x`/`dynmat.x` 使用的 tensor 来源不清 | polar correction 文件链不一致 | `fildyn`、`flfrc`、record |
...
| 状态 | 条件 | 允许进入的下游 |
|---|---|---|
| PASS | 绝缘体条件、`epsil`、dielectric tensor、Born charge、tensor symmetry / sum-rule 检查、LO-TO/non-analytic 设置和收敛均一致 | 进入 polar phonon、IR/Raman 或介电响应解释 |
| WARN | 小带隙、部分占据、tensor 对设置敏感，或 Born charge 异常但物理/数值来源尚未完全区分 | 只做响应风险说明或敏感性测试 |
| BLOCK | 在金属中解释 Born charge/dielectric 分支；response 未收敛；tensor/IFC 文件链混用；未记录 non-analytic correction 就解释 LO-TO | 停止极性响应结论 |
...
- 把 Born charge 当普通静态离子价
- 忽略 smearing 对极性响应的破坏
- tensor 异常时只给物理解释，不检查收敛、对称性和文件链
- 把 non-analytic correction 当作自动适用的后处理开关
...
- 能避免把 Born effective charge 当作普通静态离子价。
