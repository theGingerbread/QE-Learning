...
本页是 [reciprocal space and Brillouin zone](reciprocal-space-and-brillouin-zone.md) 与 [crystal symmetry, space group and point group](crystal-symmetry-space-group-point-group.md) 的工作流接口页：倒空间页解释 mesh/path/q-point 的坐标语言，对称性页解释 space group、point group 和 little group；本页只说明这些概念如何进入 QE 的 input/output 审阅。
...
在 QE workflow 里，这个区别通常表现为两个不同的 `pw.x` 任务边界：`calculation='nscf'` 常用于在已收敛 SCF density 上计算更密的 uniform mesh，以支撑 DOS、PDOS、Fermi surface 或其他 BZ 积分型后处理；`calculation='bands'` 则沿 high-symmetry k-path 计算 eigenvalues，用于 band dispersion 图。二者可以读取同一个上游 SCF 数据链，但 k 点集合、目标 observable 和 output review 证据不同，不能互相替代。

Symmetry 是连接结构和倒空间采样的另一层信息。空间群描述晶体整体对称性，点群描述绕某一点的旋转、镜面和反演等操作；这些操作在倒空间中会把不同 k 点联系起来。QE 利用这些对称性减少实际计算的 irreducible k-points。若结构 relax / vc-relax 后晶胞变形，或 SOC、磁性、noncollinear 设置改变 Hamiltonian 的对称性，原来的 k-path、简并解释、labels 和 irreducible k-point 数量都需要重新审阅。
...
Little group 是保持某个 k 点不变的对称操作集合。它是 degeneracy 和 label 依赖 k 点的直接原因：不同 k 点的 little group 可能不同；同一 k 点在 SOC、noncollinear、magnetism、`nosym` / `noinv` 或结构标准化改变后，可用的 Hamiltonian symmetry 也可能不同。QE 用户不需要完整群论推导，但需要知道：简并、band crossing、phonon irreps、bands labels 和 SOC/magnetism 下的 symmetry reduction 都不是纯绘图问题，而会影响 output review 和下游解释。
...
| `calculation='nscf'` + dense uniform mesh | `pw.x` | DOS/PDOS/Fermi surface 等积分型后处理的 eigenvalues | dense k-point summary、occupations、Fermi energy |
| `calculation='bands'` + path | `pw.x` | 沿 high-symmetry path 的 dispersion | bands k-point sequence、path labels |
...
相关 workflow：[k-point convergence](../workflows/ground-state/kpoint-convergence.md)、[bands](../workflows/electronic/bands.md)、[DOS](../workflows/electronic/dos.md)、[Fermi surface](../workflows/electronic/fermi-surface.md)、[phonon dispersion](../workflows/phonon/phonon-dispersion-dfpt.md)。相关理论回查：[reciprocal space and Brillouin zone](reciprocal-space-and-brillouin-zone.md)、[band structure and DOS](band-structure-and-dos.md)、[crystal symmetry, space group and point group](crystal-symmetry-space-group-point-group.md)。如果只需要坐标、BZ 和 q/k 语言，回到倒空间页；如果只需要 space group、point group、little group 和 symmetry labels，回到晶体对称性页。

## 核心概念

Uniform k mesh 是 Brillouin zone 积分网格。即使 QE 只实际计算 irreducible k-points，完整 mesh 与 symmetry weights 仍定义了积分。`calculation='nscf'` 的 dense uniform mesh 继承上游 SCF density，服务 DOS、PDOS、Fermi surface 和其他积分型后处理。Bands path 是 reciprocal space 中若干线段的采样；`calculation='bands'` 沿这条 path 显示 eigenvalue dispersion，不覆盖整个 BZ，不能用于 total energy、DOS、Fermi surface 或 phonon 前置积分收敛。

Symmetry 决定 QE 能否约化 k points，也影响 band degeneracy、phonon irreps、SOC/magnetism 下的可比性。更具体地说，labels 和 degeneracy 依赖当前 k 点的 little group，也依赖当前 Hamiltonian 实际保留的 symmetry。Symmetry 变化不是单纯输出格式变化，而可能改变计算对象。

## 对 input 的影响

- SCF、relax、NSCF、DOS 和 Fermi surface 应明确 uniform mesh 和 shift flags。
- `calculation='nscf'` 应与 dense uniform mesh、上游 SCF density、`nbnd` 和目标后处理绑定审阅；不要把 bands path 当作 NSCF mesh。
- `calculation='bands'` 应记录 k-path 来源、坐标基底、labels、path 段和当前 cell convention；不要把 dense NSCF mesh 当作 high-symmetry path。
- k-point convergence 中每次只改变 mesh 或按计划改变 shift，其他 cutoff、smearing、structure 和 symmetry 设置保持可归因。
- SOC、noncollinear 或磁性计算可能需要重新审阅 symmetry、little group、degeneracy 和 k-path labels。
- relax / vc-relax 后结构或 cell 改变时，应重新确认 k mesh density、reciprocal axes 和 bands path。
...
| `nscf` dense k-point summary | dense uniform mesh 被读取 | high-symmetry labels 或 band path 正确 |
...
- 结构 relax / vc-relax 后不重新确认 k-path、reciprocal axes 和 labels。
- 只记录 irreducible k-points，不记录完整 mesh 和 shift。
- 关闭 `nosym` / `noinv` 后不记录原因。
- SOC / magnetism 后仍沿用无 SOC/无磁性的 symmetry、little group、degeneracy 和 labels 解释。
...
| 状态 | 理论依据 | 下游准入 |
|---|---|---|
| PASS | mesh、shift、symmetry、irreducible k-points、target observable convergence、current cell convention、post-relax/SOC/magnetism review 和 k-path 来源可追踪 | 可进入对应 SCF/NSCF/DOS/bands/phonon 下游 |
| WARN | mesh 对部分目标尚未收敛；k-path 来源或 symmetry 关闭理由不完整；relax / vc-relax / SOC / magnetism 后沿用旧 path 但影响尚未审阅 | 可探索，不应给定量 DOS/Fermi surface、band gap、degeneracy 或 label 结论 |
| BLOCK | 用 path 代替 uniform mesh；cell/k-path convention 错配；symmetry 输出异常未解释；k-point scan 无法归因；结构或 Hamiltonian symmetry 改变后仍把旧 path / labels 当作当前证据 | 不允许进入相关下游结论 |

## 下游影响

- `SCF/relax/vc-relax`：total energy、force、stress 和 Fermi energy 依赖 k mesh。
- `NSCF/DOS/PDOS/Fermi surface`：通常需要 dense uniform mesh。
- `bands`：需要与当前 structure/cell convention 和 Hamiltonian symmetry 一致的 high-symmetry path。
