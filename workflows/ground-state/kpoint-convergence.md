...
- 上游依赖：<structure> + <pseudo> + cutoff policy
- 下游用途：SCF / relax / DOS / phonon k-point policy
- 规范入口：[standards/calculation-record-template.md](../../standards/calculation-record-template.md)、[standards/pass-warn-block.md](../../standards/pass-warn-block.md)

## 计算目标

评估 `<k_mesh>` 对总能、力、应力、Fermi 附近态或目标性质的影响，避免用未收敛采样进入下游。
...
|---|---|---|---|---|
| `K_POINTS automatic` | pw.x | 定义 uniform Brillouin zone sampling | 用 bands path 做 SCF 收敛 | output 显示 k 点和 irreducible k-points |
| `occupations` | pw.x | 影响金属体系 k 点收敛表现 | 固定占据用于金属 | output 显示 occupation scheme |
| `nosym / noinv` | pw.x | 影响对称性和 k 点约化 | 未记录对称性开关 | output 显示 symmetry operations |
