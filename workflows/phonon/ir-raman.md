...
- 上游依赖：Gamma phonon + response settings
- 下游用途：IR/Raman activity interpretation
- 规范入口：[standards/calculation-record-template.md](../../standards/calculation-record-template.md)、[standards/pass-warn-block.md](../../standards/pass-warn-block.md)

## 计算目标

整理 IR/Raman 相关响应性质的 QE 输入边界、输出字段和适用条件。
...
```text
<Gamma phonon + response settings>
  -> ph.x, `dynmat.x`
  -> <intermediate_state>
  -> <reviewed_output>
```
...
|---|---|---|---|---|
| `epsil` | ph.x | 介电和有效电荷分支 | 不检查绝缘体条件 | ph output |
| `lraman` | ph.x | 请求 Raman 相关量 | 未核对编译/功能支持和适用条件 | ph output Raman 段落 |
| `fildyn` | ph.x/dynmat.x | Gamma dynamical matrix | 读取错误 dyn 文件 | dynmat output |
